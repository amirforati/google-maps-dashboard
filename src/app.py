...
from data import combine_reviews, load_manual_reviews, load_takeout_file, parse_takeout_reviews, save_manual_review
...
def apply_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rating_min, rating_max = st.sidebar.slider("My rating", 1, 5, (1, 5))
...
def add_review_form():
    with st.expander("➕ Add review manually", expanded=False):
        ...
        with st.form("manual_review", clear_on_submit=True):
            ...
            c4, c5 = st.columns(2)
            category = c4.selectbox("Category", ["Restaurant / Food", "Attraction", "Shopping", "Hotel", "Service", "Healthcare", "Other"])
...
def explorer_page(df: pd.DataFrame):
    ...
    if selected_points:
        ...
        for point in selected_points:
            ...
            if isinstance(customdata, (list, tuple)) and customdata:
                # row_id is the final custom field produced by hover_data above.
                row_ids.append(str(customdata[-1]))
        if row_ids:
            selected = df[df.index.astype(str).isin(row_ids)]

    st.markdown(f"**Showing {len(selected):,} review(s)**" + (" from the map selection" if len(selected) != len(df) else ""))

    display = selected[["place_name", "rating", "cuisine", "city", "state", "review_date", "review_text", "google_maps_url"]].copy()
    ...
    if len(selected) == 1:
        ...
        st.caption(" · ".join(x for x in [row.get("cuisine", ""), row.get("city", ""), row.get("state", "")] if x))
...
def heatmap_page(df: pd.DataFrame):
    ...
    weight_mode = st.radio("Intensity", ["Review density", "Rating weighted"], horizontal=True)
    heat = geo[["latitude", "longitude", "rating"]].copy()
    heat["weight"] = 1 if weight_mode == "Review density" else heat["rating"]
...
def categories_page(df: pd.DataFrame):
    ...
    st.caption("Cuisine/type is an initial rules-based classification from place names and your review text. You can refine it later with Google Places enrichment or manual overrides.")
    metrics(df)
    if df.empty:
        return
    ...
    summary = (
        ...
        .agg(reviews=("place_name", "size"), average_rating=("rating", "mean"), five_star_share=("rating", lambda x: (x == 5).mean()))
        .reset_index()
        .sort_values("reviews", ascending=False)
    )
    c1, c2 = st.columns(2)
    c1.plotly_chart(px.bar(summary.head(20), x="reviews", y="cuisine", orientation="h", title="Most reviewed types"), use_container_width=True)
    c2.plotly_chart(px.bar(summary.head(20).sort_values("average_rating"), x="average_rating", y="cuisine", orientation="h", range_x=[1, 5], title="Average of my ratings"), use_container_width=True)
...
def ratings_page(df: pd.DataFrame):
    ...
    c1, c2 = st.columns(2)
    rating_counts = df["rating"].value_counts().sort_index().rename_axis("rating").reset_index(name="reviews")
    c1.plotly_chart(px.bar(rating_counts, x="rating", y="reviews", title="Rating distribution"), use_container_width=True)
    ...
    c2.plotly_chart(px.line(trend, x="year", y="reviews", markers=True, title="Reviews by year"), use_container_width=True)

    st.markdown("### Review library")
    library = df.sort_values("review_date", ascending=False)[["review_date", "place_name", "rating", "cuisine", "city", "state", "review_text"]].copy()
    library["review_date"] = library["review_date"].dt.date
    st.dataframe(library, use_container_width=True, hide_index=True, column_config={"review_text": st.column_config.TextColumn("Review", width="large")})
